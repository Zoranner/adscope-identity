...
// ===== Legacy contract types kept during the MVP migration =====
...
// ===== MVP contract types =====
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct MvpOrganizationalUnit {
    pub id: String,
    pub name: String,
...
pub struct DirectoryBatch {
    ...
    pub organizational_units: Vec<MvpOrganizationalUnit>,
    pub users: Vec<MvpUser>,
    pub groups: Vec<MvpGroup>,
    pub has_more: bool,
}
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct MvpPasswordChangeRequest {
    pub current_password: String,
    pub new_password: String,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct MvpPasswordChangeResponse {
    pub employee_id: String,
    ...
    pub fn try_from_batch(
        ...
        operations.extend(
            batch
                .users
                .iter()
                .filter(|user| user.status == MvpUserStatus::Active)
                .map(|user| {
        ...
        for user in batch
            .users
            .iter()
            .filter(|user| user.status == MvpUserStatus::Disabled)
        {
...
fn ordered_organizational_units(
    organizational_units: &[MvpOrganizationalUnit],
) -> Result<Vec<&MvpOrganizationalUnit>, DirectoryPlanError> {
...
fn visit_organizational_unit<'a>(
    ou: &'a MvpOrganizationalUnit,
    by_id: &BTreeMap<&str, &'a MvpOrganizationalUnit>,
    visiting: &mut BTreeMap<&'a str, bool>,
    ordered: &mut Vec<&'a MvpOrganizationalUnit>,
