...
struct RecordingControlPlane {
    sync_response: Arc<AgentSyncResponse>,
    confirms: Arc<Mutex<Vec<AgentConfirmRequest>>>,
...
impl RecordingControlPlane {
    ...
    fn with_batches(directory: DirectoryBatch, credentials: CredentialBatch) -> Self {
        Self {
            sync_response: Arc::new(AgentSyncResponse {
                directory,
                credentials,
...
fn directory_kind_name(kind: adss_contract::DirectoryOperationKind) -> &'static str {
    match kind {
        adss_contract::DirectoryOperationKind::EnsureOu => "ensure_ou",
        adss_contract::DirectoryOperationKind::EnsureUser => "ensure_user",
        adss_contract::DirectoryOperationKind::EnsureUserPlacement => "ensure_user_placement",
        adss_contract::DirectoryOperationKind::EnsureGroup => "ensure_group",
        adss_contract::DirectoryOperationKind::EnsureGroupMembers => "ensure_group_members",
        adss_contract::DirectoryOperationKind::DisableUser => "disable_user",
        adss_contract::DirectoryOperationKind::MoveUserToQuarantine => "move_user_to_quarantine",
    }
}

fn temp_state_path(name: &str) -> std::path::PathBuf {
    std::env::temp_dir().join(format!(
        "adss-agent-{name}-{}-{}.json",
        std::process::id(),
