...
用户密码通过中心服务修改，并由各域 Agent 同步到本地域控。域内 AD 的普通 Change Password 路径不作为事实源，也不会传播到其他域。
...
Agent key 和 LDAP bind password 按域独立保存。Agent key 运行时不得写入日志、错误响应或配置仓库。

## 传输要求

主服务必须位于 TLS 后面。Agent 调用 `/api/agent/sync` 获取凭据材料时，响应不能经过明文 HTTP，代理、网关、日志、tracing、错误回显和崩溃 dump 不记录响应体。
