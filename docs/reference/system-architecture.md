...
系统以中心服务作为账号与目录事实源，把 OU、用户、组、组成员关系和密码同步到多个独立 AD 域。每个域部署一个 Agent，Agent 主动从中心服务拉取期望状态并写入本地域控。

架构优先保证主链清晰：

- 中心数据库保存权威事实。
- 主服务负责 API、鉴权、密码保护和同步控制面。
- Agent 只执行本域同步，不反向修改中心事实。
- 目录和凭据分通道同步，互不阻塞。
- 同步基于当前状态，不重放历史事件。
...
主服务不主动连接域控，也不要求访问 Agent 内网地址。Agent 保存本地域的本地 revision state 和 Agent key，通过固定轮询访问中心服务。
...
`organizational_units` 保存中心 OU 树。节点使用稳定 `id` 和 `parent_id` 表示父子关系。Agent 在域配置的 `mirror_root_dn` 下创建对应 OU 层级。

`users` 保存用户目录事实。`employee_id` 是跨域唯一标识；`username`、显示名、邮箱、手机、办公电话、目标 OU 和状态属于目录通道。禁用用户仍是受管用户，Agent 先确保用户存在，再禁用并移动到 `quarantine_ou_dn`。
...
`domains` 保存域配置、Agent key hash、启用状态和该域已确认的目录/凭据 revision。
...
系统不保存目录变更日志、凭据变更队列、旧对象状态或旧密码。对象在 Agent 离线期间被多次修改时，Agent 只收到最新完整状态。
...
- Agent 同步：按域拉取目录和凭据批次，并确认执行结果。

普通用户 token 只进入 `/api/me/*`。管理员接口必须使用独立管理员身份，不能复用普通用户 token。Agent 接口只接受域绑定 Agent key，并只能访问该域的配置和同步数据。

rebuild 由 Agent 主动携带标志触发。

## 边界外能力

以下能力不进入目录同步主链，只有出现明确需求时才单独建模：
