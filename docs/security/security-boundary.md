...
Agent key 是域绑定共享密钥，不等价于 mTLS。部署时应通过 Windows DPAPI、Secret Manager、受限配置文件或等价机制保存 Agent key。Agent key 不得进入日志、错误响应或配置仓库。

Agent key 轮换必须记录操作人、目标域、生成时间和失效规则。接口只在生成或轮换响应中返回一次明文 key。
...
域、OU、用户、组、管理员代设密码和同步状态查询接口都属于管理面。管理员代设密码不要求用户当前密码，因此必须保留可追溯记录，并且不能复用普通用户自助改密的安全语义。

`/api/admin/*` 不得在缺少管理入口保护的情况下暴露给公网或普通用户入口。受控运维脚本可以直接调用内部能力，但必须能追溯操作者、目标对象和变更结果。
...
以下能力涉及额外生命周期和责任记录要求，应按独立安全设计处理：
